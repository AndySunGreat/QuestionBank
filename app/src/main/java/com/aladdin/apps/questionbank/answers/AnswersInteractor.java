...
public interface AnswersInteractor {
    ...
    // 当用户点击“错题”按钮进行复习时
    // 1.将order记录状态置回"FIXED",将answerID添加到answerIDs.
    // 2.生成新的answer数据
    // 当用户点击"重新做一遍"按钮时，
    // 1.将order记录状态置回"AGAIN",将answerID添加到answerIDs.
    // 2.生成新的answer数据
    interface  OnAnswerAgainFinishedListener{
        void onAnswerAgainFinished(Order items);
    ...
    void doAnswerAgain(OnAnswerAgainFinishedListener listener, JSONObject jsonObjectParam, Context context);
